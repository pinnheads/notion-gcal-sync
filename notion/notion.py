        ...
        case "rich_text":
            try:
                return [
                    ...
                    for data in task_property.get("rich_text")
                ]
            except Exception:
                return None
...
def create_event_from_tasks(task):
    ...
    event = {
        ...
        "icon": task.get("icon").get("emoji") if task.get("icon") else None,
    }
    for task_property in properties:
        event[task_property.lower().replace(" ", "_")] = get_property_value(
            property_type=properties.get(task_property).get("type"),
            task_property=properties.get(task_property),
        )
    return event
    ...
    def get_events(self, event_type: str):
        if event_type.lower() == "new":
            return self.query_db(
                query_filter=self.new_event_filter, db_id=self.notion_db_id
            )
        elif event_type.lower() == "update":
            return self.query_db(
                query_filter=self.update_event_filter, db_id=self.notion_db_id
            )
        elif event_type.lower() == "delete":
            return self.query_db(
                query_filter=self.delete_event_filter, db_id=self.notion_db_id
            )
        else:
            raise ValueError
