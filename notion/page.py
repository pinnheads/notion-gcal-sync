...
class Page(Signals, Properties):
    ...
    def get_page(self, id: str):
        return self.send_request(endpoint_url=self.notion_page_url+f"/{id}", request_type="get")

    def update_page(self, id, req_body):
        return self.send_request(endpoint_url=self.notion_page_url+f"/{id}", request_type="patch", request_body=req_body)
        ...
        req_body = {
             "properties": {
                'notion_id': {
                    'rich_text': [{
                        'type': 'text',
                        'text': {'content': notion_id}
                    }]
                },
                 'gcal_id': {
                     'rich_text': [{
                         'type': 'text',
                         'text': {'content': gcal_id}
                     }]
                 }
             }
        }
