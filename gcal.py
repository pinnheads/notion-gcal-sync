...
class TaskToEvent:
    def __init__(self, task):
        ...
        self.minutes_before_popup_reminder = (
            task.get("minutes_before_popup_reminder") or 10
        )
        ...
        if self.reminders:
            for reminder in self.reminders:
                if reminder.lower() == "email":
                    reminders.append(
                        EmailReminder(
                            minutes_before_start=self.minutes_before_email_reminder
                        )
                    )
                elif reminder.lower() == "popup":
                    reminders.append(
                        PopupReminder(
                            minutes_before_start=self.minutes_before_popup_reminder
                        )
                    )
                else:
    ...
    def create_recurrence(self):
        if self.recurring_task:
            by_month = (
                self.by_month
                if self.by_month is not None and self.by_month < 13
                else None
            )
            count = self.count
            ...
            freq = (
                convert_freq_day(self.frequency) if self.frequency != "CUSTOM" else None
            )
            if freq is not None:
                return [
                    Recurrence.rule(
